...
export default function Children() {
    ...
    // Función para obtener libros para niños de la base de datos
    const fetchChildren = async () => {
        try {
            setLoading(true);
            setError(null);
            
            // Obtener libros para niños (offset 45 para obtener diferentes libros)
            const response = await bookManagementService.getBooks({ limit: 12, offset: 45 });
            
            if (response.success && response.data.books) {
                setBooks(response.data.books);
            } else {
                setError('No se pudieron cargar los libros para niños');
            }
        } catch (err) {
            console.error('Error al cargar libros para niños:', err);
            setError('Error al cargar los libros para niños');
        } finally {
            setLoading(false);
        }
    };
        ...
        return (
            <div className="tw:mb-8 tw:px-16">
                ...
                <Typography variant="body1" sx={{ color: 'white', textAlign: 'center', py: 4 }}>
                    No hay libros para niños disponibles
                </Typography>
            </div>
        );
    }
